...
Itration_total = 0     # number of units that are used with the progress bar
Itration_count = 0    # current progress up to a maximum of "total"
...
def main():
    global Itration_count, Itration_total, CHUNK
    ...
    if ADD_SOURCE_COLUMN:
        ...
        child_data = add_header(
            child_data, SOURCE_COLUMN_NAME, CHILD_SOURCE_VALUE)
    ...
    for master_id in master_data.index:
        ...
        #printProgressBar(   Itration_count, Itration_total, length=50)


def gui():
    global Itration_count, Itration_total
    layout = [[sg.Text('Log')],
              [sg.Output(size=(80, 12))],
              [sg.Text('Progress'), sg.ProgressBar(
                  Itration_total, size=(40, 20), orientation='h', key='-PROG-')],
              [sg.Button('Run'), sg.Button('Exit')]]

    window = sg.Window('Parse Csv', layout)
    thread = None
    while True:
        ...
        if event in (None, 'Exit'):
            ...
            thread = threading.Thread(
                target=main, args=(), daemon=True)
            thread.start()
        if thread:
            window['-PROG-'].update_bar(Itration_count, Itration_total)
